...
Dependency pinning makes sure that the website does not break as a result of dependencies.
This is important especially when some dependencies do not respect semantic versioning.
For security updates, Dependabot is still configured to give security alerts when specific
...
This document intends to outline the strategy we have when adding dependencies. Only add them when explicitly needed. The more dependencies, the harder it gets to review and understand the complexity of the Website Application. You can avoid adding new dependencies if possible.
